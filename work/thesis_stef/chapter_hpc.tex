...
\item[Procedure level parallelism] larger program parts (mostly
  program procedures) are run in parallel.
\end{descriptopn}

Choice of grain size involves a tradeoff: a fine-grained 
program provides considerable flexibility with respect to load
balancing, but it also involves 
a high overhead of processing time due to a larger management need and
communication 
expense. Thus, the available hardware architecture has to be taken
into consideration. For 
example, a program running in parallel at the instruction level would
not benefit much in 
a system with distributed memory where the cost for data transfer via
a network is high 
relative to the runtime of the instruction itself. 

\subsubsection{Types  of Parallelism}
\begin{description}
\item[Implicit parallelism]
\item[Explicit parallelism with implicit decomposition]
