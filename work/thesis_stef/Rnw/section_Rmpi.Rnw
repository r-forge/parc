...
\pkg{Rmpi} management and query functions:
...
\begin{Example} Simple queries using \pkg{Rmpi}\newline
...
\pkg{Rmpi} provides the following high level functions (this is only a
selection):
...
\item[\code{.mpi.scatter(x, type, rdata, root = 0, comm = 1)}]
  ...
  from a
  specified member of the group (mostly the root) where \code{count}
