...
%Example~\ref{eq:monteCarloserial} using a path length of $50$ steps
%and 50 simulation runs. In each simulation run the mean of 5000 simulated outcomes
%is calculated. Eventually the mean of the 50 estimated option prices is
%returned (see also the routine from Example~\ref{ex:mcs}).
...
%## build example option
%european
%monteCarloSimulation(european, 0.045, n = 30, length = 5000,)
%
%@
