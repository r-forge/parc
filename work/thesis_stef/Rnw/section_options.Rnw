...
This yields to the Black-Scholes formulas for the prices at time zero
...
function \code{blackscholesprice(x, r)} to retrieve the exact price where
...
european <- define.option(c(0.1,0.4,100),100,1/12)
bsprice <- blackscholesprice(european, 0.1)
bsprice
...
now use the function \code{monteCarloSimulation()} to estimate the
...
simulated payoffs is calculated 
...
<<echo = TRUE, eval = TRUE>>=
## build example option
priced <-monteCarloSimulation(european, 0.1, n = 30, length = 5000)
priced
@
...
for(i in 1:length(nsim)){
  t <- system.time(tmp <- monteCarloSimulation(european, 0.1, 30, 5000, n.sim = nsim[i]))[3]
  est <- priceof(tmp)
  conf <- c(est-1.96*sd(pricepath(tmp))/sqrt(nsim[i]),est+1.96*sd(pricepath(tmp))/sqrt(nsim[i]))
...
mcs.Rmpi <- function(x, r, n, length, n.simulations=50,
