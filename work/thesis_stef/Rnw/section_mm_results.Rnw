...
way (see Table~\ref{tab:mm_s2500} and Figure~\ref{fig:mm_s2500}.
...
Without using the BLAS the MPI routine from package
...
Table~\ref{tab:mm_n2500} shows the best result of the matrix
multiplication benchmark for each parallel programming model.
...
table_n2500 <- cbind(X[,c(2,6)],speedup(X))
colnames(table_n2500) <- c("Type","Time","Speedup")
...
table_n5000 <- cbind(X[,c(2,6)],speedup(X))
colnames(table_n5000) <- c("Type","Time","Speedup")
xtable(table_n5000, caption="Comparison of parallel programming models on a cluster using 20 nodes and complexity grade $5000$.", label="tab:mm_s2500")
@ 
