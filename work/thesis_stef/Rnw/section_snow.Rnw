...
(\pkg{snow}---\cite{rossini03snow}, \cite{tierney07snow}) is to provide a
simple parallel computing environment in R. To make up a virtual
cluster in R \pkg{snow} is based on three different message passing
environments.
