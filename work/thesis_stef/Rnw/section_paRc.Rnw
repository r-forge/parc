...
\item[is_parallel] is a logical \code{TRUE} or \code{FALSE} wether the
...
\begin{description}
\item[\code{create.benchmark(task, data, type, cpu_range, ...)}]
  ...
  parallel paradigm to use. The \code{cpu_range} specifies the range
...
\item[\code{speedup(x)}] is a generic function taking an object as
  ...
  \code{speedup.numeric} and \code{speedup.bm_results}. The methods
...
\item[\code{plot.bm_results}] supplies a plot method for comparing
  benchmark results.
...
\item[\code{speedup.numeric}] returns the speedups calculated from a
  ...
  \class{numeric}. 
\item[\code{speedup.bm_results}] calculates the speedups from an object
  of class \class{bm_results} and returns them as a vector of type
  \class{numeric}. 
\end{description}
...
\item[\code{mcs.Rmpi(x, r, n, length, n.simulations = 50,
    antithetic = TRUE, n_cpu = 1, spawnRslaves = FALSE)}] is the same
  like \code{monteCarloSimulation()} but uses parallel generation of
  option prices. Additional arguments \code{n_cpu} and
