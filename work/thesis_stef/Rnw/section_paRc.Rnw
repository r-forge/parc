...
The package paRc (\cite{theussl07paRc}) contains high-level functions to OpenMP compiled
parallel functions.
...
paRc provides the following high level OpenMP functions: 
...
\begin{description}
\item[\code{benchmark.avail.cpu}]
\item[\code{benchmark.cpu.range}]
\item[\code{benchmark.data}]
\item[\code{benchmark.functions.to.apply}]
\item[\code{benchmark.is.parallel}]
\item[\code{benchmark.task}]
