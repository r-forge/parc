...
\item[\code{time_usr}, \code{time_sys}, \code{time_ela}] contain the
  measured runtimes 
...
Example~\ref{ex:sampleframe} shows an initialized results data
frame. The task to run with 1 and 2 processors is ``matrix
