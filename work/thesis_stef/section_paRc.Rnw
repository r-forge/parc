...
\begin{description}
\item[\texttt{omp_get_num_procs()}] returns the number of threads
  available to the program.
\item[\texttt{omp_set_num_threads()}] sets the number of threads to be
  used in subsequent parallel executions.
\item[\texttt{omp_get_max_threads()}] gets the number of threads to be
  used in subsequent parallel executions.
\end{description}
...
\item[\code{run\_benchmark(x)}] takes a benchmark object as argument
  ...
  benchmark\_results containing the
...
\item[\code{speedup.benchmark\_results}] calculates the speedups from a given
  object of \\class~\class{benchmark\_results} and returns them as a vector of
  type~\class{numeric}. 
\end{description}
...
\item[\code{Monte\_Carlo\_simulation(x, r, n, length, ...)}] carries out a
...
\item[\code{mcs.Rmpi(x, r, n, length, n\_cpu = 1, spawnRslaves =
    FALSE, ...)}] is the same
  like \code{Monte\_Carlo\_simulation()} but uses parallel generation of
