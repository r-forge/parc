...
/**************************************************************************
 * SUMMA algorithm using two dimensional processor template.
 **************************************************************************/
#include <stdio.h>
#include <math.h>
#include <sys/types.h>
...
void MULnode(  int argc, char *argv[] ){
  ...
  int temp_send_a[ONED_ARRAY];
  int temp_send_b[ONED_ARRAY];
  int temp_recv_a[ONED_ARRAY];
  int temp_recv_b[ONED_ARRAY];
  int block_a[ONED_ARRAY];
  int block_b[ONED_ARRAY];
 ...
 /* initialize the result matrix */
  ...
  for( loop = 0; loop< matrix_size; loop++){
    if( my_node == HOST ){
      ...
      /* set temporary array to send a column A and a row B */
      k=0;
      for( i = 0; i < matrix_size; i++){
	block_a[k] = ele_a[i*matrix_size+loop];
	block_b[k] = ele_b[loop*matrix_size+i];
	k++;
      }
      /* broadcast A and B */
	...
	for( i=j*num_processor_x; i< (j+1)*num_processor_x; i ++){
	  if( i != 0 ){
	    MPI_Send( &block_a[j* block_size_y],
		      block_size_y,
	...
	for( i=j; i< num_processors; i+= num_processor_x){
	  MPI_Send( &block_b[j* block_size_x],
		    block_size_x,
    ...
    */
	...
	temp_result[i*block_size_x+j] += block_a[i] * block_b[j];
	/*
	printf( " %d*%d=%d", block_a[i], block_b[j],
