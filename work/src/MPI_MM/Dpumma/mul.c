...
void MULnode( int argc, char *argv[] ){
  ...
  d_length = matrix_size * matrix_size / num_processor_x / lcm;
  ...
  for( shift = 0; shift < lcm; shift++ ){
    ...
    if( my_node == HOST ){
      k=0;
      printf("D ");
      for( l=0; l< matrix_size/lcm; l++)
	for( j=0; j< lcm; j++){
	  for( i = 0; i < matrix_size; i+= lcm ){
	    diagonal[k] = ele_b[(i+j+shift)%matrix_size*matrix_size+j + lcm*l];
	    printf(" %d", diagonal[k]);
	    k++;
	  }
	}
	...
	*/	
    ...
    if( my_node == 0 )
      printf("\n");
    /* multiply */
    /* multiplication step when 4, 9, or 16 processors running */
    if( num_processors == 4 || num_processors ==9 || num_processors == 16)
      for( i =0; i < block_size_y; i ++){
	for( j=0; j < block_size_x; j ++){
	  for( k= 0; k < block_size_x; k ++){
	    temp_result[i*block_size_x+j] += 
	      block_a[i*block_size_x+k] * temp_d0[matrix_size/lcm*j+k];	 
	    /*    if( my_node == 0 )
	...
	for( j=0; j < block_size_x; j ++){
	  for( k= 0; k < matrix_size/lcm; k ++){
	    if( j%2==0 ){
	      temp_result[i*block_size_x+j] += 
		block_a[i*block_size_x+k*2] * temp_d0[j*matrix_size/lcm+k];
	      /* 
	      if( my_node == 0 )
		printf( " %d*%d=%d(i:%d,j:%d,k:%d)", 
			block_a[i*block_size_x+k*2],
			temp_d0[j*matrix_size/lcm+k],
			temp_result[i*block_size_x+j],
			i,j,k);
	      */
	    }
	    /* multiplication of the minor row and minor diagonal */
	      ...
	      temp_result[i*block_size_x+j] += 
		block_a[i*block_size_x+2*k+1]*temp_d0[j*matrix_size/lcm+k];
	      /*if( my_node == 0 )
		printf( " %d*%d=%d(i:%d,j:%d,k:%d)", 
   ...
   /* multiplication step when 9 processors running */
	...
	for( j = 0; j < block_size_x; j ++){
	  for( k= 0; k < matrix_size/lcm; k ++){
	    if( j%3==0 ){
	      temp_result[i*block_size_x+j] += 
		block_a[i*block_size_x+k*3] * temp_d0[j*matrix_size/lcm+k];
	      /* 
	      if( my_node == 0 )
		printf( " %d*%d=%d(i:%d,j:%d,k:%d)", 
			block_a[i*block_size_x+k*2],
	    ...
	    /* multiplication of the minor row and minor diagonal */
	      ...
	      temp_result[i*block_size_x+j] += 
		block_a[i*block_size_x+3*k+1]*temp_d0[j*matrix_size/lcm+k];
	      /*if( my_node == 0 )
		printf( " %d*%d=%d(i:%d,j:%d,k:%d)", 
			block_a[i*block_size_x+2*k+1],
			temp_d0[j*matrix_size/lcm+k],
	      ...
	      temp_result[i*block_size_x+j] += 
		block_a[i*block_size_x+3*k+2]*temp_d0[j*matrix_size/lcm+k];
	      /*if( my_node == 0 )
		printf( " %d*%d=%d(i:%d,j:%d,k:%d)", 
			block_a[i*block_size_x+2*k+1],
			temp_d0[j*matrix_size/lcm+k],
