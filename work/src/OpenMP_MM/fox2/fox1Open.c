...
#pragma omp parallel private (loop, i, j,id ) shared ( matrix_size, result, block_size,ele_a,ele_b )
   {
...
#pragma omp barrier
	 ...
	 for( j =0; j< block_size; j++){
	   ...
	   result[(i*num_processor_x)*matrix_size + j*num_processor_x+id] +=
	     ele_a[(i*num_processor_x)*matrix_size +j*num_processor_x+id] *
	     ele_b[(j*num_processor_x+loop+id)%matrix_size*matrix_size + j*num_processor_x+id];
	 }
       }
	 ...
	 for( j =0; j< block_size; j++){
	   result[(i*num_processor_x+1)*matrix_size + j*num_processor_x+id%2] +=
	     ele_a[(i*num_processor_x+1)*matrix_size +j*num_processor_x+id%2] *
	     ele_b[(j*num_processor_x+loop+id%2)%matrix_size*matrix_size + j*num_processor_x+id%2];
