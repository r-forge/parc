/********************************************************
 * FileName: dimmaOpen.C
 * 
 * This coding implements Dimma algorithm using OpenMP. 
 ********************************************************/
#include <stdio.h>
#include <math.h>
#include <sys/types.h>
...
void main( int argc, char *argv[] )
{
   ...
   */ 
   ...
   /*multiplication */
#pragma omp parallel private (id,i,j,k, loop ) shared ( matrix_size, result,block_size,ele_a,ele_b )
   { 
	 ...
	 for( j =0; j< block_size; j++){
	   ...
	   for( k=0; k< matrix_size; k+=lcm ){
		 ...
		 ele_a[i*matrix_size+k+loop] *
		 ele_b[(k+loop)*matrix_size +(id *block_size+j)  ];
	     }
	     
	     /* calucation of P2, and P3 processor */
	     else{
	       result[(i+block_size)*matrix_size + ((id-2) *block_size+j)] += 
		 ele_a[(i+block_size)*matrix_size+k+loop] *
		 ele_b[(k+loop)*matrix_size +((id-2) *block_size+j)  ];
	     }
