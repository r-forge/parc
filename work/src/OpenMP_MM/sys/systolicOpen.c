/********************************************************/
/*             systolic algorithm with OpenMP           */    
/********************************************************/
#include <stdio.h>
#include <math.h>
#include <sys/types.h>
...
#pragma omp parallel private (k,id,i,j,loop) shared ( matrix_size, result, block_size,ele_a,ele_b )
    { 
	...
	for(i=0; i<matrix_size; i++)
	  for(j=0; j<matrix_size; j++){
	    temp_b[j*matrix_size+i]=ele_b[i*matrix_size+ j ];
	  }
	...
	for( i =0; i< block_size; i++ ){
	  for( j =0; j< block_size; j++ ){
	    for( k=0; k< matrix_size; k++){
	      result[(id*block_size+i)*matrix_size + ((id+loop)*block_size+j)%matrix_size] += 
		ele_a[(id*block_size+i)*matrix_size +k]*
		ele_b[(id*block_size+j)*matrix_size +k];
	    }
	  }
	}
#pragma omp barrier
	 ...
	 for(i=0; i<matrix_size; i++)
	   for(j=0; j<matrix_size; j++){
	     temp_b[i*matrix_size+j]=ele_b[(i+block_size)%matrix_size*matrix_size+ j ];
	   }
